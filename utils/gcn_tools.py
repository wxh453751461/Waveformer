...
# 根据informer论文中的ETTh1数据集，包括7个列数据，生成一个7*7的全1邻接矩阵
def get_ETTh1_adjacency_matrix():
    return np.full([7, 7], 1)

# 根据informer论文中的ETTm1数据集，包括7个列数据，生成一个7*7的全1邻接矩阵
def get_ETTm1_adjacency_matrix():
    return np.full([7, 7], 1)

# 根据informer论文中的ETTm2数据集，包括7个列数据，生成一个7*7的全1邻接矩阵
def get_ETTm2_adjacency_matrix():
    return np.full([7, 7], 1)


# 根据informer论文中的ETTh2数据集，包括7个列数据，生成一个7*7的全1邻接矩阵，其中，第7列数据异常，作为孤立点，无边
def get_ETTh2_adjacency_matrix():
    adj = np.full([7, 7], 1)
    # adj[:, 5] = 0
    # adj[5, :] = 0
    # adj[5, 5] = 1
...
# 根据informer论文中的ECL数据集，包括12个列数据，生成一个12*12的全1邻接矩阵
def get_ECL_adjacency_matrix():
    return np.full([321,321], 1)

def get_GEN_adjacency_matrix():
    return np.full([4,4], 1)

# 根据informer论文中的weather数据集，包括321个列数据，生成一个321*321的全1邻接矩阵
def get_WTH_adjacency_matrix():
    return np.full([12,12], 1)

def get_PEMS03_adjacency_matrix():
    return np.full([358,358], 1)
...
def load_graphdata_normY_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE,
                                  batch_size, shuffle=True, percent=1.0):
    '''
...
#         sw.add_scalar('RMSE_all_points', rmse, epoch)
#         sw.add_scalar('MAPE_all_points', mape, epoch)
def get_EXC_adjacency_matrix():
    return np.full([8,8], 1)


def get_ILI_adjacency_matrix():
    return np.full([7,7], 1)

def get_Traffic_adjacency_matrix():
    return np.full([100,100], 1)